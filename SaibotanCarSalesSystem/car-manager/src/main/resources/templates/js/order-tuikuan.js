(function (angular) {
    var a = angular.module("ordertuikuan", []);
    ...
    a.controller("orderTuiKunCon", function ($scope, $http, $location) {
        //所有订单里边只有查询
        ...
        $scope.tuihuoone = function(ind,obj,orderid){
            layer.confirm('确认要退款吗？',function(index){
                $.ajax({
                    type: 'get',
                    url: "/order/tuikuanone?orderid="+orderid,
                    dataType: 'json',
                    success: function(data){
                        if(data.code==1){
                            $(obj).parents("tr").remove();
                            layer.msg(data.msg,{icon:1,time:1000});
                            layer.alert("<span style='color: green'>退款成功</span>");
                            $scope.selectOrderByStatus( $scope.state);
                        }else{
                            layer.alert("<span style='color: red'>退款失败,请稍后再试</span>");
                        }
                    },
                    error:function(data) {
                        layer.alert("<span style='color: red'>退款失败,请稍后再试</span>");
        ...
        $scope.tuokuanall = function () {
            layer.confirm('确认要一键退款吗',function(index){
                var f = $http.get("/order/tuikuanall");
                f.success(function (result) {
                    if(result.code==1){
                        layer.alert("<span style='color: green'>退款成功</span>");
                    }else{
