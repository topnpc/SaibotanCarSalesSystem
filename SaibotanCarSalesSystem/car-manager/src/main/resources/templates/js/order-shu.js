(function (angular) {
    ...
    a.controller("orderShuCon", function ($scope, $http, $location) {
        //所有订单里边只有查询
        ...
        $scope.selectByTime = function () {
            ...
            if(flag!=1){
                ...
                var f = $http.get("/order/time/allok?begintime="+$scope.begintime+"&endtime="+$scope.endtime);
                f.success(function (result) {
                    if(result.code==1){
                        $scope.orderitems=result.data;
                        $scope.countPage();
        ...
        //结束订单
        $scope.jiehsuone = function(ind,obj,orderid){
            layer.confirm('确认要结束吗？',function(index){
                $.ajax({
                    type: 'get',
                    url: "/order/closeone?orderid="+orderid,
                    dataType: 'json',
                    success: function(data){
                        if(data.code==1){
                            $(obj).parents("tr").remove();
                            layer.msg(data.msg,{icon:1,time:1000});
                            layer.alert("<span style='color: green'>订单结束成功</span>");
                            $scope.selallok();
                        }else{
                            layer.alert("<span style='color: red'>订单结束失败,请稍后再试</span>");
                        }
                    },
                    error:function(data) {
                        layer.alert("<span style='color: red'>订单结束失败,请稍后再试</span>");
        ...
        //结束所有订单
        $scope.jieshuall = function () {
            layer.confirm('确认要一键结束吗',function(index){
                var f = $http.get("/order/closemore");
                f.success(function (result) {
                    if(result.code==1){
                        layer.alert("<span style='color: green'>订单结束成功</span>");
                        $scope.selallok();
